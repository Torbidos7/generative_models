...
from latent_diffusion_tf.source.denoiser import get_unet 
from latent_diffusion_tf.source.losses import l1_loss
...
class conditionalUnet(tf.keras.models.Model):
    '''
    A conditional U-Net model for image reconstruction.
    ...
    def generate_batch(self, images, class_labels):
        '''
        Generates a batch of images from a given batch of images and class labels.

        Args:
            images (tf.Tensor): The input images.
            class_labels (tf.Tensor): The class labels.

        Returns:
            generated (tf.Tensor): The generated images.
        '''
        generated = self.cUnet([images, class_labels], training=False)

        return generated

    def generate_single(self, image, class_label):
        '''
        Generates an image from a given image and class label.
        ...
        generated = self.cUnet([images, classes], training=False)
    ...
    def generate_embedding(self, image, class_label):
            '''
            Generates an embedding from a given image and class label.
            ...
            embedding = intermediate_layer_model.predict([image, classes], batch_size=1, steps=1)
    ...
    def train_step(self, data):
        ...
        images, classes, labels = data

        with tf.GradientTape() as tape:

            pred_images = self.cUnet(
                [images, classes], training=True
            )
